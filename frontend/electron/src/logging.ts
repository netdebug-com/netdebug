...
function setupRemoteLogging(url: string, device_uuid: string) {
  const ws = new WebSocket(url, {
    ...
    options: {
      headers: {
        ...
        "X-Netdebug-Device-Uuid": device_uuid,
      },
    },
  });
...
export function setupNetdebugLogging(url: string, device_uuid: string) {
  ...
  if (!isDev || OVERRIDE_LOG_TO_REMOTE) {
    ...
    setupRemoteLogging(url, device_uuid);
  } else {
